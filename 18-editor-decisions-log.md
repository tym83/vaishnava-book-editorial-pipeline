...
Status: approved
Type: STYLE
Scope: курсив на дефисных композитах с глоссарным термином
Decision: если глоссарный термин с `italic_required` входит в дефисный композит, курсивится весь композит целиком, включая элемент-имя сампрадаи/традиции (например, `рамануджа-садху` курсивом полностью).
Exceptions: абзацы стилей `Шлока` и `Шлока в цитате` не трогаем; имя ачарьи как самостоятельное слово вне композита курсивом не выделяется.
Source: прогон книги Vamshidasa Babaji, обсуждение случая `рамануджа-садху`.
Updated files: `docx_glossary_italicizer.py`.
Notes: обоснование — `рамануджа-садху` означает «садху, принадлежащий к Рамануджа-сампрадае», то есть указание на принадлежность к традиции, а не на саму личность Рамануджи. Реализуется флагом `--include-compounds`.
...
Status: approved
Type: SCRIPT
Scope: исправление OCR-ошибок в русском DOCX
Decision: OCR-ошибки не правятся слепым find/replace. Детекция — с опорой на корпус (confusion-пары к частотной форме; склейка переносов по корпусной частоте), применение — только по явной, проверенной человеком карте `[{from,to}]`.
Exceptions: спорные/стихотворные случаи и имена собственные выносятся на ручное решение; метод высокоточный, но не полный — длинный хвост закрывается ручной вычиткой или сверкой с эталоном.
Source: прогон книги Vamshidasa Babaji; локальный PDF `VDB_text_..._to_press.pdf` оказался английским оригиналом и эталоном для русского OCR не является.
Updated files: `docx_ocr_corrector.py`, `10-script-specs.md`, `requirements.txt` (опциональный `spylls`).
Notes: `detect` никогда не меняет файл; перед `apply` всегда бэкап и отчёт замен; после — повторный `docx_style_audit.py`.
...
Status: approved
Type: STYLE
Scope: поэмы, переводы поэм и внутритекстовые цитаты шлок
Decision: отдельные стихотворные строки (поэмы) получают абзацный стиль `Шлока`; их построчные/блочные переводы — `Основной текст`; шлоки или их части, процитированные внутри прозаического абзаца, выделяются символьным стилем `Char Курсив` (транслит = курсив по BBT).
Exceptions: дефис внутри стихотворной строки не трогаем (например, `гаура-кишора` в строке стиля `Шлока`); границы вкраплений и диапазоны стиха/перевода подтверждаются человеком (план для `docx_inline_verse_styler.py`).
Source: прогон книги Vamshidasa Babaji (поэмы «Бхаката-ватсала Хари» и «Шри Вамшидаса Смаране» со структурой «стих + нумерованный перевод»).
Updated files: `docx_inline_verse_styler.py`, `10-script-specs.md`, `05-style-guide.md`.
Notes: попутно унифицирует существующую рассогласованность, когда строки одной поэмы были размечены частью `Шлока`, частью `Основной текст`.
