...
## 14.2. Скрипт: docx-glossary-italicizer

## Назначение

Применять символьный стиль `Char Курсив` к глоссарным терминам с политикой `italic_required` (парный «аппликатор» к детектору `glossary_expected_italic` в `docx_style_audit.py`).
...
- читает политику глоссария (`italic_automation == always`), можно ограничить по `--entry-id`;
- ставит **символьный стиль** `Char Курсив` (не прямой `w:i`) на словоформы термина (`approved_form` + `allowed_forms`);
- границы слов как в audit (`\w`, регистронезависимо), регистр исходного текста сохраняется;
- пропускает абзацы стилей шлок (`--skip-style`, по умолчанию `Шлока`, `Шлока в цитате`);
- пропускает уже-курсивные участки;
- `--include-compounds`: расширяет совпадение на весь дефисный композит (например, `рамануджа-садху` целиком — см. Decision 013), с поддержкой span'ов через несколько run'ов.

## 14.3. Скрипт: docx-ocr-corrector

## Назначение

Детекция и исправление OCR-ошибок в русском DOCX без слепых замен. Два шага.
...
- высокоточные предложения с опорой на корпус, файл не меняет;
- confusion-ошибка буквы (н↔в↔я↔и, с↔е, ш↔щ): редкий unknown-токен, у которого одна типичная замена даёт частотную/валидную форму в этом же документе;
- разрыв переносом: дефисный токен, склейка которого частотна в документе, а половины — фрагменты;
- требует `spylls` + hunspell-словарь (`--dict`, по умолчанию `/usr/share/hunspell/ru_RU`);
- результат — JSON-предложения для ручной вычитки (метод высокоточный, но **не полный**).

## `apply`

- применяет явную, проверенную человеком карту `[{"from": .., "to": ..}]`;
- whole-token замена через границы run'ов с сохранением стилей и `Char Курсив`;
- регистр первой буквы берётся из исходного токена;
- отчёт: счётчики по правкам, `not_found`, пропуски не-простых run'ов.

## Правило

`detect` никогда не правит текст; `apply` работает только по утверждённой карте. Так чувствительный текст не правится скрытно (см. Decision 014).

## 14.4. Скрипт: docx-inline-verse-styler

## Назначение

Размечать поэмы и внутритекстовые цитаты шлок по проверенному плану.
...
- план — явный JSON (`shloka_ranges`, `body_ranges`, `italic_spans`); границы стихов/переводов и точные спаны не угадываются на этапе применения;
- непустые абзацы в `shloka_ranges` → стиль `Шлока`;
- непустые абзацы в `body_ranges` (переводы поэм) → стиль `Основной текст`;
- `italic_spans` → `Char Курсив` на точной подстроке (через run'ы), уже-курсивные участки не трогает;
- индексы абзацев 0-based по `.//w:body/w:p` (как у детектора стихов).

## Правило

Стихи-поэмы получают абзацный стиль `Шлока`, их переводы — `Основной текст`. Вкрапления шлок в прозу выделяются `Char Курсив` (транслит = курсив по BBT). См. Decision 015.
